...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className="w-64 h-full flex flex-col bg-black/40 border-r border-white/5 backdrop-blur-xl transition-all duration-300">
      {/* Brand */}
      <div className="p-6 flex items-center gap-3">
        ...
        <span className="text-xl font-black tracking-tighter uppercase bg-clip-text text-transparent bg-gradient-to-r from-white to-white/60">
          DevHub
        </span>
      </div>
        ...
        {NAV_ITEMS.map((item) => (
          ...
          >
            <div className="flex items-center gap-3">
              <item.icon className={`w-4 h-4 ${activeItem === item.id ? 'text-primary' : 'text-muted-foreground group-hover:text-white'}`} />
              {item.label}
            </div>
            {activeItem === item.id && (
      ...
      <div className="p-4 border-t border-white/5 flex flex-col gap-4">
        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 transition-colors cursor-pointer group">
          <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center border border-white/10">
            <User className="w-4 h-4 text-muted-foreground group-hover:text-white" />
          </div>
          <div className="flex flex-col">
            <span className="text-xs font-bold text-white leading-none mb-1">Luis Farfan</span>
