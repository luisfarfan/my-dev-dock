...
export const SettingsWindowPage: React.FC = () => {
  const { settings, installedEditors, fetchInstalledEditors, setDefaultEditor } = useDashboard();
  ...
  return (
    <div className="box-border h-dvh w-screen bg-black/30 px-4 pb-4 pt-8">
      <div className="desktop-glass flex h-full min-h-0 flex-col overflow-hidden rounded-2xl">
        <div className="flex h-10 shrink-0 border-b border-white/5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto py-5 pl-[82px] pr-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 className="text-lg font-black uppercase tracking-[0.2em] text-white">Settings</h2>
              <p className="text-xs text-muted-foreground">Preferencias globales del workspace local</p>
            </div>
          </div>

          <section className="space-y-3">
            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white/70">Default Editor</h3>
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-xl border px-4 py-3 flex items-center justify-between transition-colors ${
                      isSelected ? 'border-primary/50 bg-primary/10' : 'border-white/10 bg-white/5 hover:border-white/25'
                    }`}
                  >
