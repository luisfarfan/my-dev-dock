...
fn build_raycast_script_content(
    ...
    let mut lines = vec![
        ...
        "# @raycast.packageName Dev Hub".to_string(),
        format!("# @raycast.icon {}", icon.trim()),
        "# @raycast.description Generated by Dev Hub".to_string(),
    ];
    ...
    if project_paths.len() == 1 {
        ...
        lines.push(r#"if command -v "$EDITOR_BIN" >/dev/null 2>&1; then"#.to_string());
        lines.push(r#"  "$EDITOR_BIN" "$PROJECT_PATH""#.to_string());
        lines.push("else".to_string());
        lines.push(r#"  open "$PROJECT_PATH""#.to_string());
