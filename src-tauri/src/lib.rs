...
fn editor_binary(editor: &str) -> &'static str {
...
fn build_raycast_script_content(
    ...
    lines.push(format!("EDITOR_BIN={}", shell_single_quote(editor_binary(editor))));
    ...
    lines.push("  if command -v \"$EDITOR_BIN\" >/dev/null 2>&1; then".to_string());
